...
if is_torch2_available():
    from .attention_processor import SSRAttnProcessor2_0 as SSRAttnProcessor, AttnProcessor2_0 as AttnProcessor, SSRAttnProcessor2_0_text as SSRAttnProcessor_text
    from .attention_processor import SSRAttnProcessor2_0_new_v1 as SSRAttnProcessor_v1, SSRAttnProcessor2_0_new_v2 as SSRAttnProcessor_v2
else:
...
class fusion_model(torch.nn.Module):
    ...
    def generate_modify(
        ...
        image_prompt_embeds, uncond_image_prompt_embeds = self.get_image_embeds(foreground_image)

        prompt_embeds = image_prompt_embeds
        negative_prompt_embeds = uncond_image_prompt_embeds
        # prompt="a girl looking at the landscape from a distance",
    ...
    def generate_modify_new(
        ...
        image_prompt_embeds1, uncond_image_prompt_embeds1 = self.get_image_embeds(background_image)

        prompt_embeds1 = image_prompt_embeds1
        negative_prompt_embeds1 = uncond_image_prompt_embeds1
        # prompt="a girl looking at the landscape from a distance",
...
class fusion_model_v1(torch.nn.Module):
    """from SSR-encoder"""
    def __init__(self, unet, image_encoder_path, device="cuda", dtype=torch.float32):
        super().__init__()
        self.device = device
        ...
        for name in unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                attn_procs[name] = SSRAttnProcessor_v1(hidden_size=hidden_size, cross_attention_dim=1024, scale=1).to(self.device, dtype=self.dtype)
        unet.set_attn_processor(attn_procs)
        adapter_modules = torch.nn.ModuleList(unet.attn_processors.values())
        self.SSR_layers = adapter_modules
        self.SSR_layers.to(self.device, dtype=self.dtype)
...
class fusion_model_v2(torch.nn.Module):
    """from SSR-encoder"""
    def __init__(self, unet, image_encoder_path, device="cuda", dtype=torch.float32):
        super().__init__()
        self.device = device
        ...
        for name in unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                attn_procs[name] = SSRAttnProcessor_v2(hidden_size=hidden_size, cross_attention_dim=1024, scale=1).to(self.device, dtype=self.dtype)
        unet.set_attn_processor(attn_procs)
        adapter_modules = torch.nn.ModuleList(unet.attn_processors.values())
        self.SSR_layers = adapter_modules
        self.SSR_layers.to(self.device, dtype=self.dtype)
...
class fusion_model_modify(torch.nn.Module):
    """from SSR-encoder"""
    def __init__(self, unet, image_encoder_path, device="cuda", dtype=torch.float32):
        super().__init__()
        self.device = device
        ...
        for name in unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                attn_procs[name] = SSRAttnProcessor_text(hidden_size=hidden_size, cross_attention_dim=1024, scale=1).to(self.device, dtype=self.dtype)
        unet.set_attn_processor(attn_procs)
        adapter_modules = torch.nn.ModuleList(unet.attn_processors.values())
        self.SSR_layers = adapter_modules
        self.SSR_layers.to(self.device, dtype=self.dtype)
